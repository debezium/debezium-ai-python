...
class TestProjectionPolicy:
    ...
    def test_both_include_and_exclude_fields(self) -> None:
        policy = TableProjectionPolicy(
            include_fields=["name", "secret"],
            exclude_fields=["secret"],
        )
        row = {"id": 1, "name": "Widget", "secret": "shh", "other": "ignored"}
        content, _ = policy.project_row(row)
        # include_fields takes precedence over exclude_fields, so include_fields dictates what goes in
        assert "name: Widget" in content
        assert "secret: shh" in content
...
class TestDocumentBuilder:
    ...
    def test_custom_sanitizer(self, insert_event: DebeziumEventModel) -> None:
        from typing import Any

        def custom_sanitizer(meta: dict[str, Any]) -> dict[str, Any]:
            return {k: "custom" for k in meta}

        builder = DocumentBuilder(sanitizer=custom_sanitizer)
        result = builder.build(insert_event)
        assert result.document is not None
        for v in result.document.metadata.values():
            assert v == "custom"

    def test_build_soft_delete(self, delete_event: DebeziumEventModel) -> None:
        builder = DocumentBuilder()
        # Without allow_soft_delete, it should return None document
        res_hard = builder.build(delete_event, allow_soft_delete=False)
        assert res_hard.document is None
        assert res_hard.is_delete

        # With allow_soft_delete, it should return a Document with _is_deleted=True
        res_soft = builder.build(delete_event, allow_soft_delete=True)
        assert res_soft.document is not None
        assert res_soft.is_delete
        assert res_soft.document.metadata["_is_deleted"] is True
        assert res_soft.document.id == res_soft.doc_id
