...
# ══════════════════════════════════════════════════════════════════════════════
# Schema models  (mirrors PR #400 models.py)
# ══════════════════════════════════════════════════════════════════════════════
...
class DebeziumEventModel(BaseModel):
    ...
    def from_json_record(cls, record: Any) -> DebeziumEventModel:
        ...
        if schema_block:
            payload_dict = _decode_json_numerics(payload_dict, schema_block)
    ...
    def extract_primary_key(self, pk_fields: list[str] | None = None) -> str:
        """
        ...
        3. Hash the entire row as a last resort.
        """
        if self.key:
            try:
        ...
        # Fallback: deterministic hash of the full row
