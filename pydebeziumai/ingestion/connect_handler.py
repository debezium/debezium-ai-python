...
Wires the ``SourceRecordExtractor`` into the PyDebeziumAI pipeline:
...
class ConnectIngestionHandler(BaseIngestionHandler):
    ...
    Matches the architecture from  ``debezium_connect.py`` +
    ...
    def _make_consumer_class(self) -> type:
        ...
        Matches  ``_create_connect_consumer()`` factory pattern.
