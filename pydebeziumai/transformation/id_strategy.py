...
def resolve_id_strategy(
    ...
    **kwargs: object,
    ...
    if isinstance(strategy, IdStrategy):
        logger.debug("Using passed ID strategy instance: %s", strategy.__class__.__name__)
        return strategy
    if strategy in _BUILTIN:
        resolved = _BUILTIN[strategy](**kwargs)
        logger.debug("Resolved ID strategy '%s' to class %s", strategy, resolved.__class__.__name__)
        return resolved
