...
A ProjectionPolicy controls:
  ...
  - Optional Jinja2-style content templates (e.g. "{name} — {description}").
...
class TableProjectionPolicy:
    ...
    Attributes:
        ...
        exclude_fields: Fields always excluded (binary blobs, secrets, etc.).
        metadata_fields: Fields copied into Document.metadata; None = all.
        content_template: Optional ``str.format_map`` template.
    ...
    def project_row(self, row: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        if self.content_template:
            try:
                page_content = self.content_template.format_map(
